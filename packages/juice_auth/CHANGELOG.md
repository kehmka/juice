...
- Declared concurrency for every auth event. Initialization, login, logout,
  refresh, and token-expiry handoff are `droppable`; user updates are
  `sequential`.
- Replaced the public `refreshInFlight` completer with framework-level
  `EventConcurrency.droppable`. Overlapping refresh triggers still collapse to
  one provider call, while consumers continue observing `isRefreshing` and the
