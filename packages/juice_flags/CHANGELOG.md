...
- Declared concurrency for every event. Initialization and refresh are
  `droppable`; fetched updates, failures, and override mutations are
  `sequential`. Overlapping refresh requests now share one remote fetch.
